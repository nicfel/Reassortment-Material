...
for (a in seq(1,length(segments)-2)){
  for (b in seq(a+1,length(segments)-1)){
    ...
    log <- list.files(path="./pairout/", pattern=paste("h1n1sea_",segments[[a]], "_", segments[[b]], "_rep", "*[012].log",sep=""), full.names = TRUE)
    log <- list.files(path="./pairout/", pattern=paste("h1n1sea_",segments[[a]], "_", segments[[b]], "_rep", "*[012].log",sep=""), full.names = TRUE)
...
plot(p.rea)
ggsave(plot=p.rea,paste("../../../Reassortment-Text/Figures/pairwise/h1n1_pairs", ".pdf" , sep=""),width=10, height=6)
