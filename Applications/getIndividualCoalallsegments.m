function [] = getIndividualCoalallsegments(virus, workingdir, nrsequences, from, to,temperature, rngval, outgroupyear)
    ...
    for r = 0 : 2
        ...
        g = fopen(['xmls/' virus 'ind_rep' num2str(r) '.xml'], 'w');
    ...
    %              fprintf(g, '\t\t<run spec="MCMC" chainLength="200000000">\n');            
            ...
            elseif contains(line, '<parameter id="popSize.t"')
                % do nothing
            elseif contains(line, '<prior id="PopSizePrior" name="distribution" x="@popSize.t">');
                % skip two lines
                fgets(f);fgets(f);
            elseif contains(line, '<operator id="NeScaler" spec="ScaleOperator"')
                % do nothing
            elseif contains(line, '<operator id="NeScaler" spec="ScaleOperator"')
                % do nothing
            elseif contains(line, '<up idref="popSize.t"/>')
                for s = 1 : length(use_segs)
                    fprintf(g, '\t\t\t\t\t\t<up idref="popSize:%s"/>\n',use_segs{s});                  
                end
            elseif contains(line, '<log idref="popSize.t"/>')
                % do nothings
            ...
            elseif ~isempty(strfind(line, 'insert_coal_prior'))
                ...
                for s = 1 : length(use_segs)                   
                    ...
                    fprintf(g, '\t\t\t\t\t<populationModel id="ConstantPopulation.t:%s" spec="ConstantPopulation" popSize="@popSize:%s"/>\n',use_segs{s},use_segs{s});
            ...
            elseif ~isempty(strfind(line, 'insert_priors'))
                ...
                for s = 1 : length(use_segs)
                    ...
                    fprintf(g, '\t\t\t\t\t\t\t\t</prior>\n');
                    fprintf(g, '\t\t\t\t\t\t\t\t<prior id="PopSizePrior.s:%s" name="distribution" x="@popSize:%s">\n', use_segs{s}, use_segs{s});
                    fprintf(g, '\t\t\t\t\t\t\t\t\t<LogNormal id="LogNormalDistributionModel.PopSize:%s" M="0.0" S="4.0" name="distr"/>\n', use_segs{s});
                    fprintf(g, '\t\t\t\t\t\t\t\t</prior>\n');
            ...
            elseif ~isempty(strfind(line, 'insert_operators'))
                ...
                for s = 1 : length(use_segs)
                    ...
                    fprintf(g, '\t\t\t\t<operator id="NeScaler.%s" spec="ScaleOperator" parameter="@popSize:%s" scaleFactor="0.5" weight="3"/>\n', use_segs{s}, use_segs{s});
