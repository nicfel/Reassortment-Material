...
for (a in seq(1,length(segments)-2)){
  for (b in seq(a+1,length(segments)-1)){
    ...
    print("i")
    log <- list.files(path="./pairout/", pattern=paste("infB_",segments[[a]], "_", segments[[b]], "_rep", "*[012].log",sep=""), full.names = TRUE)
    log <- list.files(path="./pairout/", pattern=paste("infB_",segments[[a]], "_", segments[[b]], "_rep", "*[012].log",sep=""), full.names = TRUE)
    print("o")
...
plot(p.rea)
ggsave(plot=p.rea,paste("../../../Reassortment-Text/Figures/pairwise/infB_pairs", ".pdf" , sep=""),width=10, height=6)
...
plot(p.rea.count)
ggsave(plot=p.rea.count,paste("../../../Reassortment-Text/Figures/pairwise/infB_count", ".pdf" , sep=""),width=10, height=6)
