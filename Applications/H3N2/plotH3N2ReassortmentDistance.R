...
fileName="combined/h3n2new.distance.txt"
...
p_hist = ggplot()+
  ...
  geom_hline(yintercept=0, linetype="dashed", 
             color = "red", size=2)+
  # geom_histogram(data = reascount, aes(obs-prior,fill="observed"),binwidth=1, alpha=0.5) +
  # geom_histogram(data = reascount.prior, aes(obs,fill="expected"),binwidth=1, alpha=0.5) +
  facet_grid(.~to) +
  scale_fill_OkabeIto() +
  theme_classic()
