...
for (a in seq(1,length(segments)-2)){
  for (b in seq(a+1,length(segments)-1)){
    ...
    print("i")
    log <- list.files(path="./pairout/", pattern=paste("h3n2_",segments[[a]], "_", segments[[b]], "_rep", "*[012].log",sep=""), full.names = TRUE)
    log <- list.files(path="./pairout/", pattern=paste("h3n2_",segments[[a]], "_", segments[[b]], "_rep", "*[012].log",sep=""), full.names = TRUE)
    print("o")
    for (j in seq(1, length(log))){
      t <- read.table(log[j], header=TRUE, sep="\t")
      # take a 10 % burn in
      t <- t[-seq(1,ceiling(length(t$Sample)/5)), ]
...
plot(p.rea)
ggsave(plot=p.rea,paste("../../../Reassortment-Text/Figures/pairwise/h3n2_pairs", ".pdf" , sep=""),width=10, height=6)
...
plot(p.rea.count)
ggsave(plot=p.rea.count,paste("../../../Reassortment-Text/Figures/pairwise/h3n2_count", ".pdf" , sep=""),width=10, height=6)
