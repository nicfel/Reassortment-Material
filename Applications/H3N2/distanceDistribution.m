...
system('rm -r distxmls');
system('mkdir distxmls');
...
for r = 0 : 2
    ...
    g = fopen(['distxmls/h3n2_' segments{a} '_' segments{b} '_rep' num2str(r) '.xml'], 'w');
    while ~feof(f)
        line = fgets(f);
                ...
                if i==a || i==b
                    ...
                    if i==a
                        ...
                        for j = 1 : length(use_seqs)
                            ...
                            fprintf(g, '\t\t<sequence id="%s.%s" taxon="%s" totalcount="4" value="%s"/>\n',...
                                 segments{i}, unique_seqs{use_seqs(j)},...
                                 unique_seqs{use_seqs(j)}, regexprep(fasta(ind).Sequence, '[ATGCU]','N'));
                        end
                    end
                    fprintf(g, '\t</data>\n');
                end
        ...
        elseif ~isempty(strfind(line, 'insert_run_header'))
             fprintf(g, '\t\t<run id="mcmc" spec="beast.core.MCMC" chainLength="2000000">\n');            
