######################################################
######################################################
# Here the inferred mean coalescent and migration
# rate ratios are plotted
######################################################
######################################################
library(ggplot2)
library(coda)
library(colorblindr)

# clear workspace
rm(list = ls())

# Set the directory to the directory of the file
this.dir <- dirname(parent.frame(2)$ofile)
setwd(this.dir)

# define the threshold of when to count an event
threshold = 0

# define the number of segments
nrSegments = 9

first = T

files <- list.files(path=paste("./combined/",sep=""), pattern="h3n2\\.distance\\..*\\.txt$", full.names = TRUE)
# for (f in seq(1,length(files))){
  # read in the file with the reassortment distances
  fileName= paste("./combined/h3n2.distance.txt",sep="")
  con=file(fileName,open="r")
  line=readLines(con) 
  close(con)
  for (i in seq(1,length(line))){
    print(paste(i, "of", length(line)) )
    
    splitline = strsplit(line[[i]], split="\t")
    firstevent=T
    remove(dat)
    c=1
    
    for (j in seq(2,length(splitline[[1]]))){
      tmp = strsplit(splitline[[1]][[j]], split=",")
      tmp2 = strsplit(tmp[[1]][[1]], split=":")
      # checks if the event is on the trunk
      isTrunk = as.logical(tmp2[[1]][[1]])
      c=c+1
      for (k in seq(2,length(tmp[[1]]))){
        tmp2 = strsplit(tmp[[1]][[k]], split=":")
        if (!is.na(as.numeric(tmp2[[1]][[3]]))){
          distance = as.numeric(tmp2[[1]][[3]])-as.numeric(tmp2[[1]][[2]])
          if (distance>threshold){
            tmp3 = strsplit(tmp2[[1]][[1]], split="-")
            seg1 = as.numeric(tmp3[[1]][[1]])
            seg2 = as.numeric(tmp3[[1]][[2]])
            new.dat = data.frame(from=seg1,to=seg2, dist = distance, trunk = isTrunk )
            if(firstevent){
              dat = new.dat
              firstevent = F
            }else{
              dat = rbind(dat,new.dat)
            }
          }
        }
      }
    }
  
    # count how many events there were between the segments
    for (a in seq(0,nrSegments-2)){
      for (b in seq(0,nrSegments-2)){
        if (a!=b){
          obs.trunk = which(dat$from==a & dat$to==b & dat$trunk)
          obs.off.trunk = which(dat$from==a & dat$to==b & !dat$trunk)
          if (length(obs.trunk)&&length(obs.off.trunk)){
            ks.results.less = ks.test(dat[obs.trunk,"dist"], dat[obs.off.trunk,"dist"], alternative = "less")
            ks.results.more = ks.test(dat[obs.trunk,"dist"], dat[obs.off.trunk,"dist"], alternative = "greater")
            
            if (ks.results.more$p.value<ks.results.less$p.value){
              new.reascount = data.frame(from=a,to=b,larger=1)
            }else{
              new.reascount = data.frame(from=a,to=b,larger=0)
            }
            if(first){
              reascount = new.reascount
              first = F
            }else{
              reascount = rbind(reascount, new.reascount)
            }
          }
        }
      }
    }
  }
# }

# get the 95% HPD
first = T
for (a in seq(0,nrSegments-2)){
  for (b in seq(0,nrSegments-2)){
    if (a!=b){
      indices = which(reascount$from==a & reascount$to==b)
      new.dat = data.frame(from=a,to=b,mean=mean(reascount[indices,"larger"]))
      if (first){
        dat=new.dat
        # dat.p=new.dat.p
        first = F
      }else{
        dat=rbind(dat,new.dat)
        # dat.p=rbind(dat.p,new.dat.p)
      }
    }else{
      new.dat = data.frame(from=a,to=b,mean=NA)
      if (first){
        dat=new.dat
        # dat.p=new.dat.p
        first = F
      }else{
        dat=rbind(dat,new.dat)
        # dat.p=rbind(dat.p,new.dat.p)
      }
    }
  }
}


segments = c("HA", "MP", "NA", "NP", "NS", "PA", "PB1", "PB2", "prior")
dat$from = segments[dat$from+1]
dat$to = segments[dat$to+1]

# reascount$from = segments[reascount$from+1]
# reascount$to = segments[reascount$to+1]

# plot the histograms
p_ks = ggplot(data=dat, aes(x=from, y=to))+
  geom_tile(aes(fill=mean))+
  geom_text(aes(label = round(mean, 2) )) +
  # facet_grid(.~to) +
  # scale_fill_OkabeIto() +
  scale_fill_gradientn(colors = c("#88419d","#b3cde3",
                                  rgb(1,1,1),rgb(1,1,1),rgb(1,1,1),rgb(1,1,1),rgb(1,1,1),rgb(1,1,1), rgb(1,1,1), rgb(1,1,1), rgb(1,1,1),  
                                  "#fdcc8a","#d7301f"),
                       limits = c(0,1), 
                       na.value = "white",
                       breaks = c(0, 0.1, 0.5, 0.9, 1), 
                       name="P(obs)>P(prior)") +
  xlab("") +
  ylab("") +
  theme_minimal()
plot(p_ks)


ggsave(plot=p_ks,paste("../../../Reassortment-Text/Figures/distance/H3N2.distance.pdf", sep=""), width=5, height=4)


