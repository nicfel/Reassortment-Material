...
for (a in seq(1,length(segments)-2)){
  for (b in seq(a+1,length(segments)-1)){
    ...
    filename = paste(path="./combined/h3n2recent.event.",segments[[a]], "_", segments[[b]], ".txt",sep="")
...
p.rea.count <- ggplot(rate, aes(x=seg1,y=seg2)) +
  geom_tile(aes(fill=sumEvents)) +
  xlab("") +
  ylab("") +
  theme_light() +
...
plot(p.rea.count)
ggsave(plot=p.rea.count,paste("../../../Reassortment-Text/Figures/pairwise/h3n2_count", ".pdf" , sep=""),width=10, height=3)
