...
for (trunk_ind in seq(1, length(trunk_val))){
  ...
  for(i in seq(1, length(viruses))){
    ...
    filename = path=paste("./",viruses[[i]] ,"/simulation/", virusname[[i]], ".sim.", trunk_val[[trunk_ind]],"trunk.txt", sep="")
    # check if file exists
    if(!file.exists(filename)){
      # run the simulation file
      xmlname = gsub(paste(trunk_val[[trunk_ind]],"trunk.txt", sep=""),"xml", filename)
      system(paste("java -jar ./../Software/Coevo.jar", xmlname))
      system(paste("mv *.trees ", "./",viruses[[i]] ,"/simulation/", sep=""))
      
      networkname = gsub(".xml", ".network.trees", xmlname)
      ...
      system(paste("java -jar ./../Software/TrunkReassortment.jar -burnin 0 -removeSegments 8 -trunkDefinition minTipDistance -minTipDistance 4",
                   networkname, gsub("network.trees", "4trunk.txt", networkname)))
      system(paste("java -jar ./../Software/TrunkReassortment.jar -burnin 0 -removeSegments 8 -trunkDefinition minTipDistance -minTipDistance 2",
                   networkname, gsub("network.trees", "2trunk.txt", networkname)))
      system(paste("java -jar ./../Software/TrunkReassortment.jar -burnin 0 -removeSegments 8 -trunkDefinition minTipDistance -minTipDistance 6",
                   networkname, gsub("network.trees", "6trunk.txt", networkname)))
  ...
  ggsave(plot=plot.all,paste("../../Reassortment-Text/Figures/trunk_rates_simulation_",   trunk_val[[trunk_ind]], ".pdf", sep=""),width=7, height=5)
}
