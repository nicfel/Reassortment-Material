...
viruses = c("H3N2", "InfB")
virus_labels = c("H3N2", "Influenza B")
virusname = c("h3n2", "infB")
...
for(i in seq(1, length(viruses))){
  ...
  filename = path=paste("./",viruses[[i]] ,"/simulation/", virusname[[i]],".sim.trunk.txt", sep="")
...
plot(p)
ggsave(plot=p,paste("../../Reassortment-Text/Figures/trunk_rates_simulation.pdf", sep=""),width=7, height=3)
